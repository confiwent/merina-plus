...
class libcomyco(object):
    def __init__(self, sess, S_INFO, S_LEN, A_DIM, LR_RATE=1e-4, ID=1):
        ...
        self.inputs, self.out, self.va_1, self.va_2, self.va_3 = self.create_network()

        self.real_out = tf.clip_by_value(self.out, 1e-4, 1.0 - 1e-4)
        ...
        self.core_net_loss = tflearn.objectives.categorical_crossentropy(
            self.real_out, self.y_
        ) + 1e-3 * tf.reduce_sum(tf.multiply(self.real_out, tf.log(self.real_out)))

        self.core_train_op = tf.train.AdamOptimizer(learning_rate=LR_RATE).minimize(
            self.core_net_loss
        )
    ...
    def create_network(self):
        with tf.variable_scope("actor"):
            ...
            split_5_flat = tflearn.flatten(
                split_5
            )  # [batch, flatten_dim] flatten_dim = 128
            ...
            dense_net_0 = tflearn.gru(merge_net, FEATURE_NUM, activation="relu")

            out = tflearn.fully_connected(dense_net_0, self.A_DIM, activation="softmax")

            return inputs, out, split_0, split_2, dense_net_0

    def predict(self, state):
        action_prob = self.sess.run(
            self.real_out,
            feed_dict={self.inputs: np.reshape(state, (-1, self.S_INFO, self.S_LEN))},
        )
        # randomly picks an action
        action_cumsum = np.cumsum(action_prob)
        bit_rate = (
            action_cumsum > np.random.randint(1, RAND_RANGE) / float(RAND_RANGE)
        ).argmax()

        return action_prob, bit_rate

    def check(self, state):
        value_1, value_2, value_3 = self.sess.run(
            [self.va_1, self.va_2, self.va_3],
            feed_dict={self.inputs: np.reshape(state, (-1, self.S_INFO, self.S_LEN))},
        )
