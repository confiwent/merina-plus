"""
Fix bugs in neural network construction 

Remove the shared parameters for the input of states 
(download time, throughput, last quality, remain chunks and buffer size)

remove the download time and throughput information as the input 

fc layers after the feature extractor
...
class Actor(torch.nn.Module):
    def __init__(self, a_dim, latent_dim, state_channels, his_dim, hidden_layers=None):
        ...
        self.FeatureExactor_MLP_1 = nn.Sequential(
            nn.Linear(self.FE_mlp_input, self.FE_output), nn.LeakyReLU()
        )
        ...
        self.FeatureExactor_TD_1 = nn.Sequential(
            nn.Linear(self.his_dim, self.FE_output), nn.LeakyReLU()
        )
        ...
        self.FeatureExactor_CNN_1 = nn.Sequential(
            nn.Conv1d(
                1, self.FE_cnn_channels, 3
            ),  # for available chunk sizes 6 version  L_out = 6 - (4-1) -1 + 1 = 3
            nn.LeakyReLU(),
        )

        self.FeatureExactor_CNN_2 = nn.Sequential(
            nn.Conv1d(
                1, self.FE_cnn_channels, 3
            ),  # for available chunk psnr 6 version  L_out = 6 - (4-1) -1 + 1 = 3
            nn.LeakyReLU(),
        )

        self.FeatureExactor_latent = nn.Sequential(
            nn.Linear(self.latent_dim, self.latent_mlp_output), nn.LeakyReLU()
        )

        modules = []

        in_channels_ = (
            self.latent_mlp_output + 4 * self.FE_cnn_channels * 2 + 3 * self.FE_output
        )
        for h_dim in self.hidden_layers:
            modules.append(
                nn.Sequential(nn.Linear(in_channels_, h_dim), nn.LeakyReLU())
            )
            in_channels_ = h_dim
        ...
        self.policy = nn.Sequential(
            nn.Linear(in_channels_, self.a_dim), nn.Softmax(dim=1)
        )
    ...
    def forward(self, states_input, latent_input):
        ...
        # video psnr
        fut_inputs_2_ = states_input[:, 11:17, -1]
        fut_inputs_2_ = fut_inputs_2_.view(-1, self.num_flat_features(fut_inputs_2_))
        fut_inputs_2_ = torch.unsqueeze(fut_inputs_2_, 1)  ## (batch, 1, 6)
        ...
        FE_cnn_1 = self.FeatureExactor_CNN_1(fut_inputs_1_)
        FE_cnn_1 = FE_cnn_1.view(-1, self.num_flat_features(FE_cnn_1))

        ## video psnr
        FE_cnn_2 = self.FeatureExactor_CNN_2(fut_inputs_2_)
