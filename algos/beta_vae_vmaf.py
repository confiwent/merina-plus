"""
In this version, the encoder encodes the network throughputs and download time

!!!!! Fix bugs in neural network construction 

!!!!! Remove the shared parameters for the input of contexts (download time, throughput)
'''
...
class BetaVAE(torch.nn.Module):
    def __init__(
        ...
        in_channels_ = (
            (self.hist_dim - 3) * self.FE_cnn_channels * self.sequence_channels
        )
    ...
    def log_gaussian_loss(self, output, target, sigma):
        ...
        log_coeff = torch.log(sigma) + 0.5 * np.log(
            2 * np.pi
        )  # or torch.log(torch.from_numpy(np.pi))
